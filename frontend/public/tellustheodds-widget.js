(function (window, document) {
  ...
  style.textContent = [
    ...
    "#tuto-root { position: fixed; z-index: 2147483647; " +
      cfg.position +
      ": 24px; bottom: 24px; }",

    /* FAB */
    "#tuto-fab { width: 58px; height: 58px; border-radius: 50%; background: " +
      T.primary +
      ";",
    ...
    /* Tooltip */
    "#tuto-tooltip { position: absolute; bottom: 68px; " +
      cfg.position +
      ": 0;",
    ...
    /* Widget panel */
    "#tuto-panel { position: absolute; bottom: 72px; " + cfg.position + ": 0;",
    "  width: 400px; height: 580px; max-height: 85vh;",
    ...
    /* Quick chips */
    "#tuto-chips { padding: 10px 14px; display: flex; flex-wrap: wrap; gap: 8px;",
    "  background: #fafbfc; border-bottom: 1px solid #f0f2f5; flex-shrink: 0; }",
    ".tuto-chip { cursor: pointer; font-size: 12px; padding: 6px 12px;",
    "  border: 1px solid #c7d2fe; background: #fff; color: #3730a3;",
    "  border-radius: 20px; font-weight: 500; transition: background .15s, border-color .15s, box-shadow .15s; }",
    ...
    "  border-radius: 8px 8px 2px 8px; box-shadow: 0 2px 8px rgba(72, 134, 255, 0.15); max-width: 72%; }",
    ...
    ".tuto-bubble.bot .tuto-ts  { color: #cbd5e0; }",
    ".tuto-empty { text-align: center; color: #64748b; padding-top: 35%; }",
    ".tuto-empty .tuto-icon { font-size: 48px; margin-bottom: 12px; opacity: .5; }",
    ".tuto-empty p { font-size: 14px; font-weight: 500; line-height: 1.6; }",
    ...
    "#tuto-textarea:focus { border-color: " +
      T.primary +
      "; background: #fff; }",
    ...
    "  cursor: pointer; transition: background .15s, transform .1s, box-shadow .15s; box-shadow: 0 2px 6px rgba(72, 134, 255, 0.2); }",
    "#tuto-send:disabled { background: #cbd5e0; cursor: not-allowed; box-shadow: none; }",
    "#tuto-send:not(:disabled):hover { transform: scale(1.06); box-shadow: 0 4px 12px rgba(72, 134, 255, 0.3); }",
    ...
    "  #tuto-panel { width: calc(100vw - 20px); " +
      cfg.position +
      ": -10px; height: 78vh; }",
    "}",
  ].join("\n");
  ...
  function addMessage(text, type, ts) {
    ...
    var html = text
      ...
      .replace(/\n/g, "<br>");
    bubble.innerHTML =
      html + '<div class="tuto-ts">' + (ts || now()) + "</div>";
  ...
  $textarea.addEventListener("input", function () {
    $send.disabled =
      !this.value.trim() || !ws || ws.readyState !== WebSocket.OPEN;
    // Auto-grow
